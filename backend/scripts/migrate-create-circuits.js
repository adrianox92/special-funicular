...
async function migrateCreateCircuits() {
  try {
    ...
    if (error1) {
      console.log('[WARN] No se pudo ejecutar via exec_sql:', error1.message);
      console.log('');
      console.log('Note: Ejecuta manualmente en Supabase SQL Editor los siguientes scripts:');
      console.log('   1. backend/scripts/create-circuits-table.sql');
      console.log('   2. backend/scripts/migrate-circuit-references.sql');
      process.exit(1);
    }

    console.log('[OK] Tabla circuits creada correctamente');
    ...
    if (error2) {
      console.log('[WARN] Error al añadir referencias circuit_id:', error2.message);
      console.log('');
      console.log('Note: Ejecuta manualmente en Supabase SQL Editor:');
      console.log('   backend/scripts/migrate-circuit-references.sql');
      process.exit(1);
    }

    console.log('[OK] Referencias circuit_id añadidas correctamente');
