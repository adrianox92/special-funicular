...
async function migratePositionTracking() {
  ...
  try {
    ...
    if (error1) {
      console.log('[WARN] Campo current_position ya existe o hubo un error:', error1.message);
    } else {
      console.log('[OK] Campo current_position añadido correctamente');
    }
    ...
    if (error3) {
      console.log('[WARN] Campo previous_position ya existe o hubo un error:', error3.message);
    } else {
      console.log('[OK] Campo previous_position añadido correctamente');
    }
    ...
    if (error4) {
      console.log('[WARN] Campo position_updated_at ya existe o hubo un error:', error4.message);
    } else {
      console.log('[OK] Campo position_updated_at añadido correctamente');
    }
    ...
    if (error5) {
      console.log('[WARN] Campo position_change ya existe o hubo un error:', error5.message);
    } else {
      console.log('[OK] Campo position_change añadido correctamente');
    }
    ...
    if (error6) {
      console.log('[WARN] Índice de circuito ya existe o hubo un error:', error6.message);
    } else {
      console.log('[OK] Índice de circuito creado correctamente');
    }
    ...
    if (error7) {
      console.log('[WARN] Índice de seguimiento ya existe o hubo un error:', error7.message);
    } else {
      console.log('[OK] Índice de seguimiento creado correctamente');
    }
    ...
    if (circuitsError) {
      console.log('[WARN] Error al obtener circuitos:', circuitsError.message);
    } else {
      ...
      for (const circuit of uniqueCircuits) {
        ...
        if (timingsError) {
          console.log(`[WARN] Error al obtener tiempos del circuito ${circuit}:`, timingsError.message);
          continue;
        }
        ...
        for (let i = 0; i < timings.length; i++) {
          ...
          if (updateError) {
            console.log(`[WARN] Error al actualizar posición para timing ${timing.id}:`, updateError.message);
          }
        }
        
        console.log(`[OK] Circuito ${circuit} procesado: ${timings.length} tiempos actualizados`);
      }
    }
    ...
    console.log('');
    console.log('Note: Los nuevos campos permitirán:');
