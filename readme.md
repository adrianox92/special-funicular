...
### SEO, Search Console y sitemap

- **Frontend (Vercel):** variable `REACT_APP_SITE_URL` = URL canónica pública **sin barra final**, p. ej. `https://www.slotdatabase.es`. Debe coincidir con la propiedad de Google Search Console y con lo que declara [`frontend/public/robots.txt`](frontend/public/robots.txt) (`Sitemap:`). El apex `https://slotdatabase.es` redirige 307 a www; las URLs absolutas (canonical, og:url, JSON-LD, sitemap) deben usar www.
- **Fichas y listados públicos:** Vercel reescribe `/catalogo`, `/en/catalog` y `/de/katalog` (y sus subrutas) a [`frontend/api/catalog-ssr.js`](frontend/api/catalog-ssr.js). Esa función pide el JSON al API público, inyecta `<title>`, description, canonical, hreflang, Open Graph, JSON-LD y un cuerpo HTML con nombre/marca/referencia/imagen y enlaces prev/next (o el listado con `<a href>`), y conserva el shell CRA para que React hidrate. No hay detección de user-agent.
- **Backend (API Express):** variable `PUBLIC_SITE_ORIGIN` = el mismo origen canónico (`https://www.slotdatabase.es`). En Render conviene dejarla en www (sin barra final); si llega el apex, el backend lo reescribe a www. Sirve para generar las URLs absolutas del XML. `GET /sitemap.xml` es un **índice**; los hijos son `/sitemap-static.xml` (home, catálogo, marcas) y `/sitemap-catalog-N.xml` (ítems, ≤2000 URLs). Cada ítem aparece una vez (locale ES) con `xhtml:link` hreflang es/en/de/x-default. En producción, si falta `PUBLIC_SITE_ORIGIN`, la ruta responde `503`.
- **Proxy del sitemap en el mismo host que la web:** [frontend/vercel.json](frontend/vercel.json) reescribe `/sitemap.xml` y `/sitemap-*.xml` a [frontend/api/sitemap.js](frontend/api/sitemap.js). Esa función obtiene el XML del backend usando, por este orden, el **origen** de `SITEMAP_BACKEND_URL` o el de `REACT_APP_API_URL`. Search Console debe usar `https://www.slotdatabase.es/sitemap.xml` (el índice).
- Tras el despliegue, en Search Console **vuelve a enviar** el sitemap: `https://www.slotdatabase.es/sitemap.xml`. El hijo antiguo `sitemap-catalog-1.xml` de 5000 URLs puede seguir fallando hasta que Google reintente o se reenvíe el índice.
