...
- **Backend (API Express):** variable `PUBLIC_SITE_ORIGIN` = el mismo origen canónico (`https://slotdatabase.es`). Sirve para generar las URLs absolutas del XML. `GET /sitemap.xml` es un **índice**; los hijos son `/sitemap-static.xml` (home, catálogo, marcas) y `/sitemap-catalog-N.xml` (ítems, ≤5000 URLs). Cada ítem aparece una vez (locale ES) con `xhtml:link` hreflang es/en/de/x-default. En producción, si falta `PUBLIC_SITE_ORIGIN`, la ruta responde `503`.
- **Proxy del sitemap en el mismo host que la web:** [frontend/vercel.json](frontend/vercel.json) reescribe `/sitemap.xml` y `/sitemap-*.xml` a [frontend/api/sitemap.js](frontend/api/sitemap.js). Esa función obtiene el XML del backend usando, por este orden, el **origen** de `SITEMAP_BACKEND_URL` o el de `REACT_APP_API_URL`. Search Console debe usar `https://slotdatabase.es/sitemap.xml` (el índice).
- Tras el despliegue, en Search Console **vuelve a enviar** el sitemap: `https://slotdatabase.es/sitemap.xml`.
