...
- **Proxy del sitemap en el mismo host que la web:** [frontend/vercel.json](frontend/vercel.json) reescribe `/sitemap.xml` a la función serverless [frontend/api/sitemap.js](frontend/api/sitemap.js). Esa función obtiene el XML del backend usando, por este orden, `SITEMAP_BACKEND_URL` (URL completa del XML en Render) o, si no existe, el **origen** de `REACT_APP_API_URL` (p. ej. con `https://tu-api.onrender.com/api` → `https://tu-api.onrender.com/sitemap.xml`). En Vercel debes tener al menos una de las dos definida; con solo `REACT_APP_API_URL` suele bastar. Search Console puede usar `https://slotdatabase.es/sitemap.xml`.
