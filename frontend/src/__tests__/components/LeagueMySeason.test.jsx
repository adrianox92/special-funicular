  ...
  useTranslation: () => ({
    t: (key, opts) => {
      const map = {
        ...
        'rules.subtitle.mySeason': 'Cómo leer la ficha',
        ...
        'rules.absent.body': 'no consume descarte',
        'rules.dnf.title': 'DNF',
        ...
        'rules.tiebreak.modes.competitions_completed': 'más filas',
        'rules.tiebreak.modes.most_wins': 'más primeros',
        'rules.tiebreak.modes.last_race_position': 'última prueba',
        'rules.tiebreak.fallback': 'Por nombre',
        'rules.override.title': 'Ajustado',
        'rules.override.body': 'Override',
        'rules.override.priority': 'override > DNS',
        'rules.scoringEvents.title': 'Qué pruebas',
        'rules.scoringEvents.closed': 'Cerrada',
        'rules.scoringEvents.runningPublished': 'En curso con tiempos',
        'rules.scoringEvents.other': 'Borrador',
      };
      return map[key] ?? key;
    },
  }),
...
describe('LeagueMySeason', () => {
  ...
  test('lista descartes, DNS y no figura', () => {
    ...
    expect(screen.getByTestId('league-rules-help-trigger')).toHaveTextContent('Cómo se calcula');
  });
});
