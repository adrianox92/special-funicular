...
const VehicleCard = ({ vehicle, onDelete, onDuplicateSuccess }) => {
  ...
  return (
    <>
      ...
      {vehicle.modified && (
        <Badge className="absolute top-2 left-2 z-10">{t('card.modifiedBadge')}</Badge>
      )}
      <div className="absolute top-2 right-2 z-10 flex gap-1">
        <Button size="sm" variant="secondary" onClick={handleDuplicate} title={t('card.duplicateTitle')}>
          <CopyPlus className="size-4" />
        </Button>
        <Button size="sm" onClick={handleDownloadSpecs} title={t('card.downloadSpecsTitle')}>
          <Download className="size-4" />
        </Button>
        <Button size="sm" variant="destructive" onClick={handleDelete} title={t('card.deleteButtonTitle')}>
      ...
      <CardContent className="p-4">
        ...
        <div className="flex flex-wrap gap-1 mt-2">
          ...
          {vehicle.digital && <Badge variant="outline">{t('digital')}</Badge>}
          {vehicle.museo && <Badge variant="secondary">{t('museum')}</Badge>}
          {vehicle.taller && <Badge variant="secondary">{t('workshop')}</Badge>}
        </div>
        <div className="mt-3 text-sm">
          {vehicle.total_price !== undefined && vehicle.total_price !== null && vehicle.total_price !== vehicle.price ? (
            ...
            <span className="text-muted-foreground italic">{t('card.priceUnavailable')}</span>
          )}
        </div>
        <div className="mt-2 space-y-1 text-xs text-muted-foreground">
          {vehicle.purchase_date && (
            <div className="flex items-center gap-1">
              ...
              {new Date(vehicle.purchase_date).toLocaleDateString(getIntlLocale())}
            </div>
          )}
