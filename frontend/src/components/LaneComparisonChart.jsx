...
/**
 * Procesa tiempos ya cargados en memoria (evita segundo GET /timings al cambiar circuito).
 * @param {Array} allTimings
 * @param {string} circuit
 */
function computeLaneData(allTimings, circuit) {
  if (!circuit || !Array.isArray(allTimings)) return {};

  const circuitTimings = allTimings.filter((t) => t.circuit === circuit);
...
const LaneComparisonChart = () => {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        setAllTimings(data);
        const uniqueCircuits = [...new Set(data.filter((t) => t.circuit).map((t) => t.circuit))];
        setCircuits(uniqueCircuits.sort());
        setTimingsLoadError(null);
      } catch (err) {
        if (cancelled) return;
  ...
  const laneData = useMemo(() => {
    ...
    const out = computeLaneData(allTimings, selectedCircuit);
    if (process.env.NODE_ENV !== 'production' && typeof performance !== 'undefined') {
      const ms = performance.now() - t0;
      const n = allTimings.filter((t) => t.circuit === selectedCircuit).length;
      // eslint-disable-next-line no-console
      console.debug(`[LaneComparisonChart] procesado circuito en ${ms.toFixed(1)}ms (${n} filas)`);
    }
    return out;
  ...
  const timingsLoading = allTimings === null;
  ...
  return (
    <Card className="h-full">
      ...
      <CardContent>
        ...
        {timingsLoadError && !timingsLoading ? (
          <Alert variant="destructive" className="mb-4">
            <AlertDescription>{timingsLoadError}</AlertDescription>
          </Alert>
        ) : null}
                            ...
                            return (
                              <TableRow key={lane}>
                                ...
                                <TableCell className="font-mono font-medium">
                                  <span
                                    className={diff.isSlower ? 'text-destructive' : 'text-green-600'}
                                  >
                                    {diff.isSlower ? '+' : ''}
                                    {diff.difference}s
                                  </span>
                                ...
                                <TableCell>
                                  <span
                                    className={diff.isSlower ? 'text-amber-600' : 'text-green-600'}
                                  >
                                    {diff.isSlower ? '+' : ''}
                                    {diff.percentage}%
                                  </span>
                                ...
                                <TableCell>
                                  <Badge
                                    variant={diff.isSlower ? 'secondary' : 'default'}
                                      ...
                                      <div className="flex items-center gap-2">
                                        <span
                                          className={`font-medium ${
                                            timeDifference.isFaster
                                              ? 'text-green-600'
                                              : timeDifference.isSlower
                                                ? 'text-destructive'
