...
const MetricCard = ({
  ...
  to,
  /** Compacta la tarjeta para tiras de KPIs (menos altura, sin tendencia). */
  compact = false,
}) => {
  const linkClassName =
  ...
  const cardContent = (
    <CardContent className={cn('flex flex-col', compact ? 'min-h-0 p-3.5' : 'min-h-[132px] p-5')}>
      <div className={cn('flex flex-1 flex-col', compact ? 'gap-2' : 'gap-3')}>
        <div className="flex items-start justify-between gap-2">
          <div
            className={cn(
              'flex shrink-0 items-center justify-center rounded-xl bg-primary/10 text-primary shadow-inner ring-1 ring-primary/10 dark:bg-primary/15 dark:ring-primary/20',
              compact ? 'size-8 [&>svg]:size-4' : 'size-11 [&>svg]:size-5',
            )}
            aria-hidden
          >
            {icon}
          </div>
          {!compact && trend && trendValue ? (
            <span
              className={cn(
        ...
        <div className="min-w-0 flex-1">
          ...
          <div
            className={cn(
              ...
              compact ? 'mt-1 text-xl sm:text-2xl' : 'mt-1.5 text-2xl sm:text-[1.65rem]',
          ...
          {renderSubtitle() && (
            <p className={cn('line-clamp-2 leading-snug text-muted-foreground', compact ? 'mt-1 text-xs' : 'mt-2 text-sm')}>
              {renderSubtitle()}
            </p>
          )}
          {hasDetails && detailsMode === 'tooltip-only' && !compact && (
