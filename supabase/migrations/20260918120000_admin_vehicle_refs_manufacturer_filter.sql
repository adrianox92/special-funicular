-- Filtro opcional p_manufacturer sobre el fabricante de garaje (texto libre).
--   * vacío / null: mismo agrupado por referencia que hasta ahora.
--   * informado: contains (trim + lower) sobre vehicles.manufacturer; los grupos
--     pasan a (referencia, fabricante) para trabajar marca a marca.
-- Comodines LIKE (%, _) y backslash se escapan.
...
DROP FUNCTION IF EXISTS public.admin_vehicle_refs_missing_catalog(integer, integer, boolean, text, text);
...
COMMENT ON FUNCTION public.admin_vehicle_refs_missing_catalog(integer, integer, boolean, text, text) IS
  'Admin: refs de garaje. Sin p_q, solo ausentes del catálogo. Con p_q, contains (trim+lower) e incluye refs ya en catálogo. p_manufacturer filtra y agrupa por marca de garaje (contains trim+lower). Solo service_role.';
